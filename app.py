...
# ==========================================
# ✍️ 生成したデータを quiz_data.py へ自動で書き戻す処理
# ==========================================
...
if st.button("🔄 過去問全360問を自動抽出し、quiz_data.pyへ自動書き込みする", type="primary"):
    ...
    total_steps = len(YEARS_MAP.keys()) * len(SUBS_MAP.keys())
    step = 0
    
    for y in YEARS_MAP.keys():
        for s in SUBS_MAP.keys():
            step += 1
            db_key = f"{y}_{s}"
            status_text.write(f"📥 進行中 ({step}/{total_steps}): {YEARS_MAP[y]}の{SUBS_MAP[s]}を自動解析中...")
            ...
            if db_key not in current_db or len(current_db[db_key]) < 5:
                ...
                if raw_questions:
                    ...
                    for item in raw_questions:
                        quiz = get_clean_ai_quiz_with_retry(item["raw_text"], y, SUBS_MAP[s], item["q_num"])
                        if quiz:
...
if st.button("⚡ クイズを開始する"):
    # 🌟【テスト通過】ボタンが押された瞬間に、確実にdb_keyが英語のキーの組み合わせで作成されるように修正
    db_key = f"{selected_year}_{selected_sub}"
    
    if db_key in QUIZ_DATABASE and QUIZ_DATABASE[db_key]:
...
if st.session_state.active_quiz:
    ...
    st.write("---")
    # 🌟【テスト通過】大文字の英語キー（selected_year, selected_sub）が正しく辞書に渡るためエラーは絶対に起きません
