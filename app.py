...
# 外部のquiz_data.pyからデータを読み込む（最初は空でも動くようにガード）
try:
...
# ==========================================
# 🔍 【核心】PDFから全問の文章を「何回も」自動スキャンして切り出す処理
# ==========================================
    ...
    for attempt in range(3):
        try:
            ...
            if "429" in str(e) or "RESOURCE_EXHAUSTED" in str(e):
                time.sleep(20) # 429エラー時は20秒自動待機 [INDEX]
                continue
    return None

# ==========================================
# ✍️ 【全自動執筆】生成したデータを quiz_data.py へ自動で追記・書き戻す処理
# ==========================================
def write_all_data_to_quiz_data_file(complete_database):
    """完成した全問本物データを、あなたのGitHubの quiz_data.py へ全自動で直接上書き保存する"""
...
if st.button("🔄 過去問全360問を自動抽出し、quiz_data.pyへ自動書き込みする", type="primary"):
    ...
    for y in YEARS:
        for s in SUBJECTS.keys():
            ...
            # まだデータが書き込まれていない試験のみを狙って、何回も自動で見に行く
            if db_key not in current_db or len(current_db[db_key]) < 5:
                ...
                if raw_questions:
                    ...
                    # 1つの試験（全15〜18問）が終わるたびに、自動でquiz_data.pyに直接上書き保存（追記）
...
if st.button("⚡ クイズを開始する"):
    # 外部のquiz_data.pyから今自動で書き込まれた本物のデータを引き出す
    if db_key in QUIZ_DATABASE:
        st.session_state.active_quiz = QUIZ_DATABASE[db_key]
        st.session_state.current_q_idx = 0
        st.session_state.is_answered = False
        st.success(f"🎯 {YEARS_MAP[selected_year]}の{SUBJECTS[selected_sub]}（全{len(st.session_state.active_quiz)}問）を正常にロードしました！")
        st.rerun()
    else:
...
if st.session_state.active_quiz:
    ...
    st.write("---")
    st.subheader(f"📝 {YEARS_MAP[selected_year]} {SUBJECTS[selected_sub]} 問{quiz['q_num']}")
